...
def get_recon_imgs_fn(opt, nfe):
    if opt.use_i3sb:
        suffix = f"i3sb_samples_nfe{nfe}_eta{opt.eta}"
        sample_dir = RESULT_DIR / opt.ckpt / suffix
    else:
        suffix = f"i2sb_samples_nfe{nfe}"
        sample_dir = RESULT_DIR / opt.ckpt / suffix
...
def compute_batch(ckpt_opt, corrupt_type, corrupt_method, out):
    ...
    return corrupt_img, x1, mask, cond, y, clean_img
...
def main(opt):
    ...
    for loader_itr, out in enumerate(val_loader):
        corrupt_img, x1, mask, cond, y, clean_img= compute_batch(ckpt_opt, corrupt_type, corrupt_method, out)

        if opt.use_i3sb:
            sv_idx = str(loader_itr).zfill(3)
            results_dir = sample_dir / f"{sv_idx}"
            # results_dir = None
            xs, pred_x0s = runner.i3sb_sampling(
                    ckpt_opt, x1, ita=opt.eta, mask=mask, cond=cond, clip_denoise=opt.clip_denoise, 
                    nfe=nfe, verbose=opt.n_gpu_per_node==1, log_count=log_count
                )
        else:
            xs, pred_x0s = runner.i2sb_sampling(
                ckpt_opt, x1, mask=mask, cond=cond, clip_denoise=opt.clip_denoise, 
                nfe=nfe, verbose=opt.n_gpu_per_node==1, log_count=log_count
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eta",            type=float, default=1,          help="i3sb stochasticity. 1.0 recovers i2sb")
    parser.add_argument("--use-i3sb",       action="store_true",            help="use i3sb")
